...
exec ActualizarUsuario 13,'Marta','Marta@uh.com','Regular','1212'
exec InsertarUsuario 'Rosa','rosa@gmail.com','Administrador','1111'
...
create procedure Insertarfactura
  ...
  SET @Fecha = GETDATE()
    insert into Factura (Fecha,total,IV_Total,Cliente) values (@Fecha,@Total,@IVTotal,@CodigoCliente)
...
begin
  DECLARE @Fecha datetime
  SET @Fecha = GETDATE()
update Factura set Fecha=@Fecha, Total=@Total, IV_Total=@IV_Total, Cliente=@Cliente
...
create procedure InsertarDETFactura
  ...
  as 
  begin
    insert into DET_Factura(Linea,Codigo_Producto,Cantidad,Precio_Unitario,IV) values (@Linea,@Codigo_Producto,@Cantidad,@PrecioUnitario,@IVA)
    end  
...
as
begin
select Codigo_DET_Factura, linea, Codigo_Producto, Cantidad, Precio_Unitario, IV from DET_Factura
end
...
as
begin 
update DET_Factura set Linea=@Linea,Codigo_Producto=@Codigo_Producto,Cantidad=@Cantidad,Precio_Unitario=@PrecioUnitario,IV=@IVA
...
as
begin
select Codigo_DET_Factura, Linea,Codigo_Producto,Cantidad,Precio_Unitario,IV from DET_Factura where Codigo_DET_Factura = @Codigo
end
-----------------------------------------------------------------------------------------------------------
